...
-   늘어난 컨텍스트 길이는 모델 메리 사용량에 영향을 미치지만 파라미터 수에는 영향을 미치지 않는다.
...
-   70억 개의 파라미터를 가지고 있고, 각 파라미터가 2바이트(16비트)를 사용한다면, 이 모델이 필요로 하는 GPU 메모리는 최소 140억 바이트(14GB)가 된다. (실제로는 더 크고 다른 장에서 사용량 계산 방법을 설명한다)
...
2.  선호도 파인튜닝: 사람의 선호도에 맞는 응답 출력하도록 파인튜닝 (RLHF : GPT-3.5, 라마 2, DPO : 라마 3, AI 피드백 기반 강화학습: 클로드)
